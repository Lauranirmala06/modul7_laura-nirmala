...
class Siswa:
    ...
    def tambah_siswa(nama, asal_sekolah):
        ...
        jumlah_kelas = (len(Siswa.daftar_siswa) // 3) + 1 #menghitung jumlah elemen dalam sebuah list# yntuk menghitung kelas setiap kelas ada 3
        kelas = f"Kelas {jumlah_kelas}"
        plotting = input("Masukkan plotting bimbingan siswa: ").strip()
    ...
    def hapus_siswa(nama):#menghapus siswa
    ...
    def tampilkan_siswa():
        ...
        kelas_dict = {}# untuk mengelompokan berdasarkan kelas
        for siswa in Siswa.daftar_siswa:
            if siswa['kelas'] not in kelas_dict:
                kelas_dict[siswa['kelas']] = []
            kelas_dict[siswa['kelas']].append(siswa)

        for kelas, siswa in kelas_dict.items():
            print(f"\n{kelas}:")
            for s in siswa:
    ...
    def perbarui_siswa(nama):
        for siswa in Siswa.daftar_siswa:
            if siswa['nama'] == nama:
                ...
                siswa['plotting'] = input("Masukkan plotting baru: ").strip()
                print(f"Siswa {nama} berhasil diperbarui.")
                return
        print(f"Siswa {nama} tidak ditemukan.")

    def menu():#Menyediakan antarmuka menu untuk pengguna
        while True:
